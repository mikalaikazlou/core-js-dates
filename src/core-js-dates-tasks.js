...
function isDateInPeriod(date, period) {
  const endDateIso = new Date(period.end).setUTCHours(0, 0, 0, 0);
  const startDateIso = new Date(period.start).setUTCHours(0, 0, 0, 0);
  const checkDate = new Date(date).setUTCHours(0, 0, 0, 0);
  return endDateIso >= checkDate && checkDate >= startDateIso;
}
...
function getWeekNumberByDate(date) {
  ...
  const correctionByMn = (date.getDay() + 6) % 7;

  currentDate.setDate(currentDate.getDate() - correctionByMn + 3);
  ...
  return (
    1 + Math.ceil((currentDate - firstThursday) / (7 * 24 * 60 * 60 * 1000))
  );
}
